...
<body>
    <div class="layout">
        <aside class="sidebar">
            ...
            @if(auth()->check())
                <div class="user-id">User ID: {{ auth()->id() }}</div>
            @endif
            @if(auth()->check())
                <a href="{{ route('dashboard') }}" class="{{ request()->routeIs('dashboard') ? 'active' : '' }}">Dashboard</a>
                @unless(auth()->user()->isAdmin())
                    <a href="{{ route('warehouses.index') }}" class="{{ request()->routeIs('warehouses.*') ? 'active' : '' }}">Warehouses</a>
                    <a href="{{ route('invitations.index') }}" class="{{ request()->routeIs('invitations.*') ? 'active' : '' }}">Invitations</a>
                @endunless
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.users') }}" class="{{ request()->routeIs('admin.*') ? 'active' : '' }}">User Management</a>
                @endif
